...
def image_noise(dataset, args):
	...
	if args.noise_info is None:
		...
		n_rand = int(args.noise_rate * n_train)
		randomize_indices = np.random.choice(range(n_train), size=n_rand, replace=False)
		print(f"n_train:{n_train}  n_rand:{n_rand} randomize_indices:{randomize_indices}")


		if args.noise_type == 'Gaussian':
			# generate examples by Gaussian distribution with the same mean and std as original dataset
			randomized_images = np.random.randn(*train_images[randomize_indices].shape).transpose(0,2,3,1)
			print("randomize_iamges:",randomized_images.shape)
			...
			randomized_images = randomized_images.transpose(0,3,1,2).astype(np.uint8)
			print("randomize_iamges:", randomized_images.shape)
